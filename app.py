...
with st.sidebar:
    ...
    home_team = st.text_input("Equipo Local", value="Brasil")
    away_team = st.text_input("Equipo Visitante", value="Argentina")

    st.markdown("### xG del Partido")
    xg_home = st.number_input(f"xG {home_team}", min_value=0.10, max_value=6.0,
                               value=1.45, step=0.05, format="%.2f")
    xg_away = st.number_input(f"xG {away_team}", min_value=0.10, max_value=6.0,
                               value=1.20, step=0.05, format="%.2f")

    st.markdown("### Promedios Copa del Mundo")
    avg_home = st.number_input("Promedio goles local (histórico)", min_value=0.5,
                                max_value=4.0, value=1.46, step=0.01, format="%.2f",
                                help="Media histórica de goles del equipo local en la Copa del Mundo")
    avg_away = st.number_input("Promedio goles visitante (histórico)", min_value=0.5,
                                max_value=4.0, value=1.06, step=0.01, format="%.2f",
                                help="Media histórica de goles del equipo visitante en la Copa del Mundo")

    st.markdown("### Hándicap Asiático")
    handicap = st.select_slider("Hándicap al local",
                                 options=[-2.0, -1.5, -1.0, -0.5, 0.0, 0.5, 1.0, 1.5, 2.0],
                                 value=0.0)

    st.markdown("---")
    st.markdown(
        "<div style='font-size:0.75rem;color:#4caf50;'>Modelo: Distribución Poisson Bivariada<br>"
        "Los valores xG se usan como λ (tasa de goles esperados).</div>",
        unsafe_allow_html=True,
    )
...
st.markdown(
    "<div class='info-box'>Todas las probabilidades se calculan mediante distribución de Poisson Bivariada "
    "usando los xG como parámetro λ. Los promedios históricos de la Copa del Mundo se usan como referencia "
    "de contexto para interpretar los xG ingresados.</div>",
    unsafe_allow_html=True,
)
...
with col_l:
    ...
    <div class='team-header' style='flex-direction:column;align-items:flex-start;'>
        ...
        <div class='metric-sub'>Promedio Copa: {avg_home:.2f}</div>
...
with col_r:
    ...
    <div class='team-header' style='flex-direction:column;align-items:flex-end;'>
        ...
        <div class='metric-sub'>Promedio Copa: {avg_away:.2f}</div>
    </div>""", unsafe_allow_html=True)
...
with col_hc:
    st.markdown(f"**Hándicap Asiático ({handicap:+.1f} al local)**")
