...
.fair-odds b { color: #A5D6A7; font-weight: 700; }
...
.ou-half { font-family: 'Space Mono', monospace; font-size: 1.15rem; color: #FFC107; font-weight: 700; }
...
.ou-tag-h { font-size: 0.62rem; color: #FFC107; text-transform: uppercase; letter-spacing: 1.5px; margin: 4px 0 2px; font-weight: 600; }
...
def clean_name(raw, fallback):
    """Elimina caracteres que romperían el HTML inyectado con unsafe_allow_html."""
    s = re.sub(r"[<>]", "", raw or "").strip()
    return s if s else fallback


def rho_bounds(lh, la):
    """Rango de rho que mantiene todos los factores tau >= 0 (Dixon & Coles)."""
    lo = -1.0 / max(lh, la)
    hi = min(1.0, 1.0 / (lh * la)) if lh * la > 0 else 1.0
    return lo, hi


def dixon_coles_matrix(lh, la, rho, n=MAX_G):
    """Matriz de probabilidad conjunta Dixon-Coles. `rho` debe venir ya validado."""
    i = np.arange(n)[:, None]
    j = np.arange(n)[None, :]
    indep = poisson.pmf(i, lh) * poisson.pmf(j, la)
    tau = np.ones((n, n))
    tau[0, 0] = 1 - lh * la * rho
    tau[0, 1] = 1 + lh * rho
    tau[1, 0] = 1 + la * rho
    tau[1, 1] = 1 - rho
    mat = np.maximum(indep * tau, 0.0)
    mat /= mat.sum()
    return mat
...
def calc_ou(mat, line):
    ...
    totals = idx[0] + idx[1]
    over = float(np.sum(mat[totals > line]))
    return over, 1.0 - over


def calc_exact_total(mat, max_g=None):
    """P(total de goles == g) para cada g, vía suma de antidiagonales."""
    n = mat.shape[0]
    total_max = 2 * n - 2
    if max_g is None or max_g > total_max:
        max_g = total_max
    flipped = np.fliplr(mat)
    return {g: float(np.trace(flipped, offset=(n - 1) - g)) for g in range(max_g + 1)}


def calc_asian_ou_full(mat, line):
    """Descompone una línea asiática de cuarto (x.25 / x.75) apostando al Over.

    Over x.75  -> gana total / gana mitad / pierde total
    Over x.25  -> gana total / pierde mitad / pierde total
    """
    frac = round(line % 1, 2)
    if frac not in (0.25, 0.75):
        raise ValueError("Solo líneas de cuarto (x.25 / x.75)")
    exact = calc_exact_total(mat)
    if frac == 0.75:
        pivot = int(line + 0.25)
        full_win = sum(p for g, p in exact.items() if g > pivot)
        half_win = exact.get(pivot, 0.0)
        return {"full_win": full_win, "half_win": half_win,
                "half_loss": 0.0, "full_loss": 1.0 - full_win - half_win}
    pivot = int(line - 0.25)
    full_win = sum(p for g, p in exact.items() if g > pivot)
    half_loss = exact.get(pivot, 0.0)
    return {"full_win": full_win, "half_win": 0.0,
            "half_loss": half_loss, "full_loss": 1.0 - full_win - half_loss}


def calc_margin(mat):
    n = mat.shape[0]
    return {d: float(np.trace(mat, offset=-d)) for d in range(-5, 6)}
...
def calc_asian_handicap(mat, H):
    """(win, push, loss) para el equipo LOCAL con hándicap H (vectorizado)."""
    n = mat.shape[0]
    diff = np.arange(n)[:, None] - np.arange(n)[None, :] + H
    win = float(mat[diff > EPS].sum())
    push = float(mat[np.abs(diff) <= EPS].sum())
    loss = float(mat[diff < -EPS].sum())
    return win, push, loss


def desc_handicap(L, negative):
    """Descripción de la condición de victoria de la apuesta (incluye push si aplica)."""
    entera = abs(L % 1) < EPS
    if negative:
        if entera:
            return f"gana por más de {int(L)} (push si gana por {int(L)})"
        return f"gana por {int(L + 0.5)}+ goles"
    if entera:
        return f"pierde por menos de {int(L)} (push si pierde por {int(L)})"
    if L == 0.5:
        return "empata o gana"
    return f"no pierde por {int(L + 0.5)}+ goles"


def top_scorelines(mat, k=10):
    n = mat.shape[0]
    flat = np.argsort(mat, axis=None)[::-1][:k]
    return [(float(mat[i, j]), int(i), int(j)) for i, j in zip(*np.unravel_index(flat, (n, n)))]
...
def fair_odds(p):
    """Cuota decimal justa (sin margen): 1/p."""
    return f"{1.0/p:.2f}" if p > EPS else "—"


def fair_odds_push(win, push):
    """Cuota justa de una apuesta con push (hándicap entero): EV=0 con reembolso.

    win·(o−1) − loss = 0  →  o = 1 + (1 − win − push) / win
    """
    if win <= EPS:
        return "—"
    return f"{1.0 + (1.0 - win - push) / win:.2f}"


def fair_odds_quarter(res):
    """Cuota justa de una línea de cuarto: EV=0 sobre las dos mitades.

    o = 1 + (½·pierde_mitad + pierde_todo) / (gana_todo + ½·gana_mitad)
    """
    denom = res["full_win"] + 0.5 * res["half_win"]
    if denom <= EPS:
        return "—"
    return f"{1.0 + (0.5 * res['half_loss'] + res['full_loss']) / denom:.2f}"
...
with st.sidebar:
    ...
    st.markdown("---")
    team1 = clean_name(st.text_input("Nombre Equipo 1 (Local)", value="Equipo 1"), "Equipo 1")
    team2 = clean_name(st.text_input("Nombre Equipo 2 (Visitante)", value="Equipo 2"), "Equipo 2")
    st.markdown("---")
    ...
    calib_mode = st.radio(
        ...
        ["Proporcional (conserva ratio)", "Aditivo (conserva diferencia)"],
        index=0,
        disabled=not adjust_avg,
        help=(
            "Proporcional: multiplica ambos xG por el mismo factor (conserva λ₁/λ₂, "
            "pero amplifica o comprime la ventaja del favorito). "
            "Aditivo: reparte la media conservando la supremacía λ₁−λ₂, que suele ser "
            "el estadístico más informativo del enfrentamiento."
        ),
    )
    rho = st.slider(
...
if adjust_avg and total_xg_input > 0:
    if calib_mode.startswith("Aditivo"):
        supremacy = xg_home - xg_away
        lambda_h = (avg_goals + supremacy) / 2
        lambda_a = (avg_goals - supremacy) / 2
        if lambda_h < MIN_LAMBDA or lambda_a < MIN_LAMBDA:
            # La supremacía excede la media objetivo: el reparto aditivo daría un λ <= 0
            scale = avg_goals / total_xg_input
            lambda_h = xg_home * scale
            lambda_a = xg_away * scale
            st.info(
                f"La diferencia de xG ({supremacy:+.2f}) es demasiado grande para repartir "
                f"aditivamente una media de {avg_goals:.2f}. Se aplicó calibración proporcional.",
                icon="ℹ️",
            )
    else:
        scale = avg_goals / total_xg_input
...
# Validación de rho FUERA de la función cacheada (st.warning dentro de una
# función con @st.cache_data no se muestra en hits de caché).
rho_lo, rho_hi = rho_bounds(lambda_h, lambda_a)
rho_used = float(np.clip(rho, rho_lo + EPS, rho_hi - EPS))
if abs(rho_used - rho) > 1e-6:
    st.warning(
        f"Con λ₁={lambda_h:.2f}, λ₂={lambda_a:.2f}, el ρ seleccionado ({rho:.2f}) genera factores τ negativos. "
        f"Se ha ajustado automáticamente a {rho_used:.2f} para mantener la validez del modelo.",
        icon="⚠️",
    )


@st.cache_data(show_spinner=False, max_entries=128)
def build_matrix(lh, la, r):
    return dixon_coles_matrix(lh, la, r, MAX_G)


mat = build_matrix(lambda_h, lambda_a, rho_used)
...
asian_lines = {L: calc_asian_ou_full(mat, L) for L in (2.25, 2.75, 3.25)}
...
btts_y_safe = max(btts_y, EPS)

exact   = calc_exact_total(mat, max_g=15)
margins = calc_margin(mat)
# Colas del margen (victorias por 6+ goles) para que la distribución sume 100%
tail_home = max(h - sum(v for k, v in margins.items() if k > 0), 0.0)
tail_away = max(a - sum(v for k, v in margins.items() if k < 0), 0.0)
...
# P(marca) coherente con la matriz DC (no con la Poisson independiente,
# que ignora el ajuste tau en los marcadores bajos)
p_e1_marca = 1.0 - float(mat[0, :].sum())
p_e2_marca = 1.0 - float(mat[:, 0].sum())

# Marginales por equipo derivadas de la matriz DC (coherentes con tau,
# a diferencia de usar la Poisson univariada directamente)
home_marg = mat.sum(axis=1)
away_marg = mat.sum(axis=0)

def team_over(marg, line):
    return float(marg[np.arange(len(marg)) > line].sum())

tt_lines = [0.5, 1.5, 2.5]
tt_home = {L: team_over(home_marg, L) for L in tt_lines}
tt_away = {L: team_over(away_marg, L) for L in tt_lines}

# Par / Impar de goles totales
_idx = np.indices(mat.shape)
_totals = _idx[0] + _idx[1]
p_even = float(np.sum(mat[_totals % 2 == 0]))
p_odd = 1.0 - p_even

# Portería a cero y "gana a cero" (win to nil)
cs_home = float(mat[:, 0].sum())          # el visitante no marca
cs_away = float(mat[0, :].sum())          # el local no marca
wtn_home = float(mat[1:, 0].sum())        # local gana sin recibir gol
wtn_away = float(mat[0, 1:].sum())        # visitante gana sin recibir gol
...
<p class='sec-desc'>Probabilidad de cada resultado al término de los 90 minutos.
La <b>cuota justa</b> es 1/probabilidad, sin margen del bookmaker: hay valor cuando la cuota
ofrecida en el mercado <b>supera</b> a la justa.</p>
""", unsafe_allow_html=True)
...
for col, (lbl, val, color) in zip(
    ...
    <div class='card {border_cls}'>
        ...
        <div class='fair-odds'>cuota justa <b>{fair_odds(val)}</b></div>
...
with col4:
    ...
    <div class='card'>
        ...
        <div class='fair-odds'>cuota justa <b>{fair_odds(float(mat[bi,bj]))}</b></div>
...
for col, (lbl, val, color) in zip(cols_dnb[:2], [
    ...
    <div class='card'>
        ...
        <div class='fair-odds'>cuota justa <b>{fair_odds(val)}</b> (empate reembolsa)</div>
        {bar_html(val*100, color)}
    </div>""", unsafe_allow_html=True)
...
<p class='sec-desc'>Probabilidad de superar o no cada línea de goles totales en el partido.
La tarjeta resaltada es la <b>línea justa</b>: aquella donde el Over queda más cerca del 50%
(el punto de equilibrio contra el que comparar la línea del mercado).</p>
""", unsafe_allow_html=True)
...
# Línea principal: la más equilibrada (Over más cercano al 50%), como cotizan los books
main_ou_line = min(ou_lines, key=lambda t: abs(t[1] - 0.5))[0]
ou_html = "<div class='ou-grid'>"
for line, ov, un in ou_lines:
    hl = "ou-card-highlight" if line == main_ou_line else ""
    ou_html += f"""
    <div class='ou-card {hl}'>
        <div class='ou-line'>{line}</div>
        <div class='ou-tag-o'>Over</div>
        <div class='ou-over'>{ov:.1%}</div>
        <div class='fair-odds'>@<b>{fair_odds(ov)}</b></div>
        <div class='ou-sep'></div>
        <div class='ou-tag-u'>Under</div>
        <div class='ou-under'>{un:.1%}</div>
        <div class='fair-odds'>@<b>{fair_odds(un)}</b></div>
    </div>"""
ou_html += "</div>"
st.markdown(ou_html, unsafe_allow_html=True)

# Línea justa de goles: total L* donde P(Over) = 50%, interpolado entre medias líneas
fair_ou_line = float(main_ou_line)
for _k in range(len(ou_lines) - 1):
    (l0, o0, _), (l1, o1, _) = ou_lines[_k], ou_lines[_k + 1]
    if (o0 - 0.5) * (o1 - 0.5) <= 0 and abs(o1 - o0) > EPS:
        fair_ou_line = l0 + (l1 - l0) * (o0 - 0.5) / (o0 - o1)
        break

st.markdown(f"""
<div class='info-pill' style='margin-top:4px;'>
    <div>Línea justa de goles <span>{fair_ou_line:.2f}</span></div>
    <div>línea asiática más cercana <span>{round(fair_ou_line*4)/4:.2f}</span></div>
    <div style='font-size:0.72rem;color:#555;'>Si el mercado cotiza una línea claramente distinta, ahí está la discrepancia con tu modelo.</div>
</div>""", unsafe_allow_html=True)
...
En las líneas enteras (−1.0, −2.0…) existe el <b>push</b> (reembolso), mostrado en amarillo.
</p>
""", unsafe_allow_html=True)
...
def handicap_card(sign, L, win, push, desc, color):
    push_row = (
        f"<div style='font-family:Space Mono,monospace;font-size:0.7rem;color:#FFC107;margin-top:4px;'>push {push:.1%}</div>"
        if push > EPS else ""
    )
    return f"""
    <div style='flex:1; background:#141414; border:1px solid #1F1F1F; border-radius:12px; padding:12px;'>
        ...
        <div style='font-family:Space Mono,monospace; font-size:1.3rem; font-weight:700; color:{color};'>{win:.1%}</div>
        {push_row}
        <div class='fair-odds'>cuota justa <b>{fair_odds_push(win, push)}</b></div>
        <div style='margin-top:6px;'>{bar_html(win*100, color=color, height=4)}</div>
    </div>"""
...
with col_loc:
    ...
    for L in handicap_lines:
        w_neg, push_neg, _ = calc_asian_handicap(mat, -L)   # local -L
        w_pos, push_pos, _ = calc_asian_handicap(mat, L)    # local +L
        st.markdown(
            "<div style='display:flex; gap:10px; margin-bottom:12px;'>"
            + handicap_card("-", L, w_neg, push_neg, desc_handicap(L, negative=True), "#4CAF50")
            + handicap_card("+", L, w_pos, push_pos, desc_handicap(L, negative=False), "#4CAF50")
            + "</div>",
            unsafe_allow_html=True,
        )
...
with col_vis:
    ...
    for L in handicap_lines:
        # Visitante -L cubre cuando el LOCAL +L PIERDE su apuesta
        _, push_neg, w_neg = calc_asian_handicap(mat, L)
        # Visitante +L cubre cuando el LOCAL -L PIERDE su apuesta
        _, push_pos, w_pos = calc_asian_handicap(mat, -L)
        st.markdown(
            "<div style='display:flex; gap:10px; margin-bottom:12px;'>"
            + handicap_card("-", L, w_neg, push_neg, desc_handicap(L, negative=True), "#4FC3F7")
            + handicap_card("+", L, w_pos, push_pos, desc_handicap(L, negative=False), "#4FC3F7")
            + "</div>",
            unsafe_allow_html=True,
        )


# Hándicap justo: H* donde el local cubre exactamente 50% (push-ajustado),
# interpolado linealmente entre líneas de 0.5 — más preciso que el argmin del grid
_h_grid = np.arange(-3.5, 3.51, 0.5)
def _ah_eff(Hv):
    w, _, l = calc_asian_handicap(mat, float(Hv))
    return w / (w + l) if (w + l) > EPS else 0.5
_effs = [_ah_eff(Hv) for Hv in _h_grid]
fair_h_line = float(_h_grid[int(np.argmin(np.abs(np.array(_effs) - 0.5)))])
for _k in range(len(_h_grid) - 1):
    e0, e1 = _effs[_k], _effs[_k + 1]
    if (e0 - 0.5) * (e1 - 0.5) <= 0 and abs(e1 - e0) > EPS:
        fair_h_line = float(_h_grid[_k] + 0.5 * (0.5 - e0) / (e1 - e0))
        break

st.markdown(f"""
<div class='info-pill' style='margin-top:4px;'>
    <div>Hándicap justo del local <span>{fair_h_line:+.2f}</span></div>
    <div>línea asiática más cercana <span>{round(fair_h_line*4)/4:+.2f}</span></div>
    <div style='font-size:0.72rem;color:#555;'>La línea de mercado más alejada de esta es donde buscar valor.</div>
</div>""", unsafe_allow_html=True)


# ═══════════════════════════════════════════════════════════════════════════════
# 6. TOTALES ASIÁTICOS (líneas de cuarto)
# ═══════════════════════════════════════════════════════════════════════════════
st.markdown("""
<div class='sec-header'>
  <span class='sec-num'>06</span>
  <span class='sec-icon'>🧮</span>
  <span class='sec-label'>Over Asiático — Líneas de cuarto</span>
</div>
<p class='sec-desc'>
Cada línea de cuarto divide la apuesta en dos mitades. Se muestra la probabilidad de cada
desenlace apostando al <b>Over</b>: gana todo, gana/pierde la mitad, o pierde todo.
</p>
""", unsafe_allow_html=True)

asian_html = "<div class='ou-grid-3'>"
for L, res in asian_lines.items():
    half_lbl = "Gana ½" if res["half_win"] > EPS else "Pierde ½"
    half_val = res["half_win"] if res["half_win"] > EPS else res["half_loss"]
    asian_html += f"""
    <div class='ou-card'>
        <div class='ou-line'>Over {L}</div>
        <div class='ou-tag-o'>Gana todo</div>
        <div class='ou-over'>{res['full_win']:.1%}</div>
        <div class='ou-sep'></div>
        <div class='ou-tag-h'>{half_lbl}</div>
        <div class='ou-half'>{half_val:.1%}</div>
        <div class='ou-sep'></div>
        <div class='ou-tag-u'>Pierde todo</div>
        <div class='ou-under'>{res['full_loss']:.1%}</div>
        <div class='ou-sep'></div>
        <div class='fair-odds'>cuota justa <b>{fair_odds_quarter(res)}</b></div>
    </div>"""
asian_html += "</div>"
st.markdown(asian_html, unsafe_allow_html=True)


# ═══════════════════════════════════════════════════════════════════════════════
# 7. TOTALES POR EQUIPO · PAR/IMPAR · PORTERÍA A CERO
# ═══════════════════════════════════════════════════════════════════════════════
st.markdown(f"""
<div class='sec-header'>
  <span class='sec-num'>07</span>
  <span class='sec-icon'>🥅</span>
  <span class='sec-label'>Totales por Equipo · Par/Impar · Portería a Cero</span>
</div>
<p class='sec-desc'>
Mercados derivados de las <b>marginales de la matriz Dixon‑Coles</b> (coherentes con el ajuste τ,
a diferencia de usar la Poisson univariada, que sesgaría los marcadores bajos).
</p>
""", unsafe_allow_html=True)

col_tt1, col_tt2 = st.columns(2)
for col, (team, tt, color) in zip(
    [col_tt1, col_tt2],
    [(team1, tt_home, "#4CAF50"), (team2, tt_away, "#4FC3F7")],
):
    with col:
        st.markdown(
            f"<div style='font-family:Space Mono,monospace;font-weight:700;color:{color};margin-bottom:12px;'>Goles de {team}</div>",
            unsafe_allow_html=True,
        )
        tt_html = "<div class='ou-grid-3'>"
        for L, ov in tt.items():
            un = 1.0 - ov
            tt_html += f"""
            <div class='ou-card'>
...
<div class='mg-grid-6'>
    <div class='mg-card'>
        <div class='card-lbl'>Total Par</div>
        <div class='card-val-sm'>{p_even:.1%}</div>
        <div class='fair-odds'>@<b>{fair_odds(p_even)}</b></div>
        {bar_html(p_even*100)}
    </div>
    <div class='mg-card'>
        <div class='card-lbl'>Total Impar</div>
        <div class='card-val-sm' style='color:#CE93D8;'>{p_odd:.1%}</div>
        <div class='fair-odds'>@<b>{fair_odds(p_odd)}</b></div>
        {bar_html(p_odd*100, "#CE93D8")}
    </div>
    <div class='mg-card'>
        <div class='card-lbl'>P. a cero {team1[:8]}</div>
        <div class='card-val-sm'>{cs_home:.1%}</div>
        <div class='fair-odds'>@<b>{fair_odds(cs_home)}</b></div>
        {bar_html(cs_home*100)}
    </div>
    <div class='mg-card'>
        <div class='card-lbl'>P. a cero {team2[:8]}</div>
        <div class='card-val-sm' style='color:#4FC3F7;'>{cs_away:.1%}</div>
        <div class='fair-odds'>@<b>{fair_odds(cs_away)}</b></div>
        {bar_html(cs_away*100, "#4FC3F7")}
    </div>
    <div class='mg-card'>
        <div class='card-lbl'>Gana a cero {team1[:8]}</div>
        <div class='card-val-sm'>{wtn_home:.1%}</div>
        <div class='fair-odds'>@<b>{fair_odds(wtn_home)}</b></div>
        {bar_html(wtn_home*100)}
    </div>
    <div class='mg-card'>
        <div class='card-lbl'>Gana a cero {team2[:8]}</div>
        <div class='card-val-sm' style='color:#4FC3F7;'>{wtn_away:.1%}</div>
        <div class='fair-odds'>@<b>{fair_odds(wtn_away)}</b></div>
        {bar_html(wtn_away*100, "#4FC3F7")}
    </div>
</div>""", unsafe_allow_html=True)
...
with col_b1:
    ...
    <div class='btts-big btts-yes'>
        ...
        <div class='fair-odds'>cuota justa <b>{fair_odds(btts_y)}</b></div>
        {bar_html(btts_y*100)}
    </div>""", unsafe_allow_html=True)
...
with col_b2:
    ...
    <div class='btts-big btts-no'>
        ...
        <div class='fair-odds'>cuota justa <b>{fair_odds(btts_n)}</b></div>
...
fig_eg = go.Figure(go.Bar(
    ...
    customdata=[fair_odds(v) for v in exact.values()],
    hovertemplate="%{x}: %{y:.1f}%<br>cuota justa %{customdata}<extra></extra>",
))
...
for lbl, lo, hi, desc in mg_ranges:
    ...
    <div class='mg-card'>
        ...
        <div class='fair-odds'>cuota justa <b>{fair_odds(p)}</b></div>
...
for rank, (prob, i, j) in enumerate(top10):
    ...
    <div class='top10-card {cls}'>
        ...
        <div class='fair-odds'>@<b>{fair_odds(prob)}</b></div>
    </div>"""
top10_html += "</div>"
...
mg_items = [(-6, tail_away)] + sorted(margins.items()) + [(6, tail_home)]
mg_x, mg_y, mg_c, mg_t = [], [], [], []
for diff, p in mg_items:
    if diff >= 6:    label = f"{team1[:6]} +6≥";  color = "#4CAF50"
    elif diff <= -6: label = f"{team2[:6]} +6≥";  color = "#EF5350"
    elif diff > 0:   label = f"{team1[:6]} +{diff}";  color = "#4CAF50"
...
fig_mg = go.Figure(go.Bar(
    ...
    textfont=dict(family="Space Mono", size=10, color="#888"),
    customdata=[fair_odds(p) for _, p in mg_items],
    hovertemplate="%{x}: %{y:.1f}%<br>cuota justa %{customdata}<extra></extra>",
))
...
━━━━━━━━━━━━━━━━━━━━━━
🏆 *RESULTADO FINAL (1X2)* _(cuota justa)_
✅ {team1} gana: {pct(h)} (@{fair_odds(h)})
🤝 Empate: {pct(d)} (@{fair_odds(d)})
❌ {team2} gana: {pct(a)} (@{fair_odds(a)})
...
⚖️ Línea justa: {fair_ou_line:.2f} | Hándicap justo local: {fair_h_line:+.2f}
...
━━━━━━━━━━━━━━━━━━━━━━
🥅 *TOTALES POR EQUIPO / EXTRAS*
{team1} Over 1.5: {pct(tt_home[1.5])} | {team2} Over 1.5: {pct(tt_away[1.5])}
Portería a cero → {team1}: {pct(cs_home)} | {team2}: {pct(cs_away)}
Gana a cero → {team1}: {pct(wtn_home)} | {team2}: {pct(wtn_away)}
Total Par: {pct(p_even)} | Impar: {pct(p_odd)}
...
_ρ={rho_used:.2f} · λ₁={lambda_h:.2f} · λ₂={lambda_a:.2f}_"""
...
<div style='text-align:center;padding:40px 0 20px;border-top:1px solid #1A1A1A;margin-top:32px;'>
    ...
    <div style='font-family:Space Mono,monospace;font-size:0.58rem;color:#2A2A2A;margin-top:6px;'>
        λ₁ = {lambda_h:.2f} ({team1}) · λ₂ = {lambda_a:.2f} ({team2}) · ρ = {rho_used:.2f} · Media {avg_goals:.2f} g/p · Total {xg_total:.2f}
    </div>
</div>""", unsafe_allow_html=True)
